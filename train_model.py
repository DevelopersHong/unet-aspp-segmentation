# -*- coding: utf-8 -*-
# @Time    : 2020-08-06 17:54
# @Author  : cai
# @Email   : caihong2050@gmail.com
# @File    : train.py
"""
...
def train_net(net, cfg):
    ...
    val_percent = cfg.validation / 100  # 0.1作为验证集
    n_val = int(len(dataset) * val_percent)  # 验证集数
    n_train = len(dataset) - n_val  # 训练集数
...
# 信息显示
    writer = SummaryWriter(
        comment=f'LR_{cfg.lr}_BS_{cfg.batch_size}_SCALE_{cfg.scale}')
    global_step = 0
    ...
    for epoch in range(cfg.epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{cfg.epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                writer.add_scalar(
                    'model/lr', optimizer.param_groups[0]['lr'], global_step)
                ...
                if global_step % (len(dataset) // (10 * cfg.batch_size)) == 0:
                    ...
                    if cfg.n_classes > 1:
                        ...
                        writer.add_scalar('CrossEntropy/test',
                                          val_score, global_step)
                    else:
                        logging.info(
                    ...
                    if cfg.n_classes == 1:
                        ...
                        inference_mask = torch.sigmoid(
                            inference_masks) > cfg.out_threshold
                        ...
                        inference_masks = torch.chunk(
                            inference_masks, ids, dim=1)
                        for idx in range(0, len(inference_masks)):
                            inference_mask = torch.sigmoid(
                                inference_masks[idx]) > cfg.out_threshold
            ...
            if cfg.deepsupervision:
                ...
                for masks_pred in masks_preds:
                    ...
                    for true_mask, pred in zip(true_masks, masks_pred):
                        ...
                        if cfg.n_classes > 1:
                            sub_cross_entropy = F.cross_entropy(pred.unsqueeze(
                                dim=0), true_mask.unsqueeze(dim=0).squeeze(1)).item()
                        else:
                            sub_cross_entropy = dice_coeff(
                                pred, true_mask.squeeze(dim=1)).item()
